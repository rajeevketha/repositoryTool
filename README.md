...
GitHub, GitLab, or Azure DevOps is **optional**. Without a team repo, snapshots live on **this device** so you can promote the same package sandbox → QA → prod. Jira is optional on that path. Choose **Release repo** (GitHub, GitLab, or Azure) when the team needs a shared warehouse — then a Jira key and comment are required.
...
1. **Start** — Detect logged-in orgs. Set **From** and **To** in the path bar (they must be different on every step — the other org is disabled in each picker). Opening OrgFlow starts a new package so the last session’s members do not come back. Stay on Start and choose **On this device** or **Release repo** (GitHub, GitLab, or Azure). Set **API version** (default **67.0**; after Detect the list comes from the org so 68/69 appear automatically). Click **Next** when that choice is done. If a release repo: follow **Help · connecting Git**, then **Use this repo**. OrgFlow saves From → To as the promotion path (for example DEC → QA). Add Staging and Prod when those orgs are logged in so the same Jira snapshot can travel Dev → QA → Staging → Prod. Pipelines are optional — connecting the repo is enough. Changing From on Package or Retrieve clears those members and requires a new retrieve. Changing To after retrieve sends you back to Retrieve.
...
3. **Retrieve** — click **Retrieve from From org** when the package is complete. When storage is **On this device**, a Jira key and comment are optional (blank saves as `CHANGE-YYYYMMDD-N`). On **Release repo**, both are required (`PROJ-123`; spaces are OK — OrgFlow turns `Proj 123` into `PROJ-123`). **Save to repo** stays off until that key looks right. After retrieve, **Download zip** saves the package to disk. **This retrieve vs saved snapshot** shows moving files on the left and a saved version on the right. **Back** returns to Package. After you add members, retrieve again before Confirm.
4. **Confirm** — review org names, Jira, comment, and what is going to the To org. **Already in To?** checks selected members against the To org (does not block Deploy). If the package includes Apex, **Run Apex tests** is optional: pick classes to run in the To org for a coverage percentage. Test classes do not have to be in the package. Skip tests and Deploy still works in a sandbox. In the workbench, the right rail shows From → To, member/file counts, a “before you send” checklist, Validate / Deploy, and the result. **Validate in To org** is a dry run (nothing is saved). **Deploy** sends it. When storage is **On this device**, a successful deploy auto-saves the snapshot so you can promote the same files to the next org. The result stays on this screen. After a successful deploy, **New package** starts over from Start.
...
- **Store upload:** [files/orgflow-chrome-web-store.zip](https://raw.githubusercontent.com/rajeevketha/repositoryTool/cursor/orgflow-salesforce-deploy-c0ed/files/orgflow-chrome-web-store.zip) (`manifest.json` at the zip root, version **1.11.25**)
...
- **On this device** — no token. Jira is optional. If you enter a key, versions (`PROJ-123-v1`) stay on this computer. Blank keys save as `CHANGE-YYYYMMDD-N`. Promote that same snapshot to QA, then prod.
- **GitHub, GitLab, or Azure** — Jira key and comment are required (the comment is the Git commit message). Same versions, written to the host you pick so teammates can reuse them. Token stays in this browser.
...
3. **Retrieve** — retrieve the package when it looks complete. When storage is **On this device**, Jira and comment are optional. On **Release repo**, both are required. **Download zip** saves the retrieved package. **Back** returns to Package unlocked so you can add more members, then retrieve again. Errors from Salesforce show in the result panel (component name + problem).
4. **Confirm** — org names, Jira, comment, and the component list. **Already in To?** is a preflight, not a gate. **Validate in To org** is a dry run. **Deploy** sends it. A successful deploy on this device auto-saves the snapshot. The Salesforce result stays on this screen.
...
- **On this device / GitHub, GitLab, or Azure** — versioning is always on. A team repo is optional sharing. Jira is required only for the repo path.
